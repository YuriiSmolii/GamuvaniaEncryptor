...
namespace CaesarEncryptor
{
    public partial class Form1 : Form
    {
        ...
        private string EncryptionV(string inputText, string typeAlphabet)
        {
            char[] alphabet = new char[0];
            string encryptText;
            ...
            for (int i = 0; i < inputText.Length; i++)
            {
                if (typeAlphabet == "English")
                {
                    vernamaKey += Convert.ToString(alphabet[rand.Next(0, 26)]);
                }
                if (typeAlphabet == "Ukrainian")
                {
                    vernamaKey += Convert.ToString(alphabet[rand.Next(0, 33)]);
                }
            }
            textBoxGaslo.Text = vernamaKey;
            byte[] bytesInputText = Encoding.Unicode.GetBytes(inputText);
            byte[] bytesKey = Encoding.Unicode.GetBytes(textBoxGaslo.Text);
        ...
        private string DecryptionV(string inputText)
        {
            string decryptText;
            byte[] bytesInputText = Encoding.Unicode.GetBytes(inputText);
            byte[] bytesKeys = Encoding.Unicode.GetBytes(textBoxGaslo.Text);
            byte[] bytesEncryptText = new byte[bytesInputText.Length];
            for (int i = 0; i < bytesInputText.Length; i++)
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (isValidText(text, alphabet))
            {
                DecryptedText.Text = EncryptionV(text, alphabet);
            }
            RichTextBoxChanged = true;
        }
        ...
        {
            string encryptText = OriginalText.Text;
            DecryptedText.Text = DecryptionV(encryptText);
            RichTextBoxChanged = true;
        }
    }
}
